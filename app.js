// ============================================
//  NOTEBOOK BOOKING SYSTEM — APP.JS
...
function validateEnglishName(val) {
  // Only English letters, spaces, hyphens, apostrophes
...
function validateForm(data) {
  ...
  if (data.printerFloors.length === 0) {
    document.getElementById('error-printer-floors').textContent = 'กรุณาเลือกอย่างน้อย 1 ชั้น';
    isValid = false;
  } else {
...
function showSuccess(ref, data) {
  ...
  successInfo.innerHTML = `
    ...
    👤 ${data.fullName}<br/>
    📞 ${data.phone}<br/>
    🖨️ ชั้นปริ้น: ${data.printerFloors.length > 0 ? data.printerFloors.map(f => 'ชั้น ' + f).join(', ') : '-'}<br/>
